...
def process_sphinx_result(sphinx_output):
    ...
    if sphinx_output == "a":
       ...
       # a가 출력되면 x=6, y=0.5 좌표로 이동하는 액션토픽 전송
        action_pub = rospy.Publisher('/move_base_simple/goal', PoseStamped, queue_size=10)
        action_msg = PoseStamped()
        action_msg.header.frame_id = "map"
        action_msg.pose.position.x = 6
        action_msg.pose.position.y = 0.5
        action_pub.publish(action_msg)
        
    elif sphinx_output == "b":
        kobuki_twist.linear.x = 0.5
        kobuki_twist.angular.z = 0.0
       # b가 출력되면 x=6.5, y=5.5 좌표로 이동하는 액션토픽 전송
        action_pub = rospy.Publisher('/move_base_simple/goal', PoseStamped, queue_size=10)
        action_msg = PoseStamped()
        action_msg.header.frame_id = "map"
        action_msg.pose.position.x = 6.5
        action_msg.pose.position.y = 5.5
        action_pub.publish(action_msg)
       ...
       # c가 출력되면 x=2.0, y=5.8 좌표로 이동하는 액션토픽 전송
        ...
        action_msg = PoseStamped()
        action_msg.header.frame_id = "map"
        action_msg.pose.position.x = 2.0
        action_msg.pose.position.y = 5.8
        action_pub.publish(action_msg)
        
    elif sphinx_output== "d":
        kobuki_twist.linear.x = 0.5
        kobuki_twist.angular.z = 0.0
       # d가 출력되면 x=0.0, y=0.0 좌표로 이동하는 액션토픽 전송
        action_pub = rospy.Publisher('/move_base_simple/goal', PoseStamped, queue_size=10)
        action_msg = PoseStamped()
        action_msg.header.frame_id = "map"
        action_msg.pose.position.x = 0.0
        action_msg.pose.position.y = 0.0
        action_pub.publish(action_msg)
